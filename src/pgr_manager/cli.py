...
from pgr_manager.config import APPID_TO_SERVER, load_app_config, save_app_config
from pgr_manager.core import WGameManager, PGRError

__version__ = metadata.version("pgr-manager")
...
def get_help_text_with_version():
    """生成帮助文本"""
    help_text = f"PGR CLI Manager (v{__version__})"
    config = load_app_config()
...
def version_callback(value: bool):
    """处理 --version 参数"""
    if value:
        typer.echo(f"pgr-manager version: {__version__}")
        raise typer.Exit()
